...
function createSubGrid(i, depth) {
    var object = new THREE.Object3D();
    object.position.x = ((i % 5) * 400) - 800;
    object.position.y = (-(Math.floor(i / 5)) * 400) + 800;
    object.position.z = -(depth + 1) * 100;
...
function makeElement(file, template) {
    var element = document.createElement('div');
    element.id = file.path
    element.className = 'element ' + file.mimeType;
    element.style.backgroundColor = 'rgba(0,127,127,' + (Math.random() * 0.5 + 0.25) + ')';
    element.innerHTML += X.render(template, file);
    return element;
}
...
function init() {
    ...
    readDir('/', function (content) {
        ...
        for (var i = 0; i < n; i++) {
            ...
            if (el.isFolder) {
                console.log("Reading", el.path)
                readDir(el.path)
                placeSubFolderinFS( , el.index, )
